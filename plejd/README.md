...
Home Assistant add-on for [Plejd](https://www.plejd.com/) home automation devices. Control your Plejd lights, switches, relays and scenes from Home Assistant over Bluetooth (BLE), with wireless buttons exposed as automation triggers and MQTT auto-discovery of the devices in range. Changes made in the Plejd app are propagated back to Home Assistant.
...
| preferCachedApiResponse | Read the device list and crypto key from the on-disk cache (`/data/cachedApiResponse.json`) instead of contacting the Plejd cloud on startup (default `false`). The cache is always used automatically as a fallback if the cloud API is unreachable; set this to `true` to prefer it even when the API is up.                                                                                                           |
...
| connectionTimeout       | Seconds to wait when scanning and connecting (default `2`). If the add-on struggles to connect on restart — a reconnect loop, `Discovery timeout elapsed`, or `Failed to start discovery. Operation already in progress` — raise this to `10`–`15`. Also make sure nothing else is using the same Bluetooth adapter (e.g. Home Assistant's own Bluetooth integration — the add-on needs exclusive access to the dongle). |
